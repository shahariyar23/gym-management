...
import { deleteToCart, fetchToCart, updateToCart } from "@/store/gyn/cartSlice";
...
const UserCartItemsContent = ({ cartItem }) => {
  ...
  const handelUpdateQuantity = (course, actionType) => {
    dispatch(
      updateToCart({
        userId: user?.id,
        courseId: course?.courseId,
        quantity:
          actionType === "plus" ? course?.quantity + 1 : course?.quantity - 1,
      })
    ).then((res) => {
  ...
  const handelDelete = (courseId) => {
    dispatch(deleteToCart({ userId: user?.id, courseId })).then((res) => {
      if (res?.payload?.success) {
        dispatch(fetchToCart({ userId: user?.id }));
        toast({
    ...
    <div className="flex md:flex-row flex-col gap-4  items-center space-x-4">
      ...
      <div className="flex flex-row md:flex-col px-5 justify-between md:items-center gap-4 w-full">
        <Button
          ...
          onClick={() => handelDelete(cartItem?.courseId)}
        >
